 ...
 *
 * this class does 0.0.9 Report to 1.0.0
 *
 ...
 * anticipated
 *
 * We assume only one report per XML message
 *
 */
 ...
 * @author jmarkpullen
 */
public class Translate009Report {
    
    String inputXml;
    String outputXml = "";
    static int ersatzReportIndex = 0;
    ...
    String translate(String xml){
        ...
        // we make up a ReportID if, per schema, there is none
        if(reportID.equals(""))reportID = nextReportID();
        ...
        "</ToReceiver><ReportContent><PositionReportContent><TimeOfObservation><DateTime>" +
        "<IsoDateTime>" + timeOfObs + "</IsoDateTime></DateTime></TimeOfObservation>" +
        "<EntityHealthStatus><OperationalStatus><OperationalStatusCode>" + statusCode +
        "</OperationalStatusCode></OperationalStatus></EntityHealthStatus>" +
        "<EntityHealthStatus><Strength><StrengthPercentage>" + strengthPercentage +
        "</StrengthPercentage></Strength></EntityHealthStatus><Location>" +
        "<GeodeticCoordinate><Latitude>" + latitude + "</Latitude><Longitude>" +
        longitude + "</Longitude></GeodeticCoordinate></Location><SubjectEntity>" +
        ...
        "</ReportingEntity></ReportBody>";
            ...
            outputXml += 
        "<ReportBody><FromSender>" + fromSender + "</FromSender><ToReceiver>" + toReceiver + 
        "</ToReceiver><ReportContent><ObservationReportContent><TimeOfObservation><DateTime>" +
        "<IsoDateTime>" + timeOfObs + "</IsoDateTime></DateTime></TimeOfObservation>" +
        "<Observation><LocationObservation><ActorReference>UNK</ActorReference>" +
        "<Location><GeodeticCoordinate><Latitude>" + latitude + "</Latitude><Longitude>" +
        longitude + "</Longitude></GeodeticCoordinate></Location></LocationObservation>" +
        "</Observation></ObservationReportContent></ReportContent>" +
        "<ReportID>" + reportID + "</ReportID><ReportingEntity>" + reportingEntity +
        "</ReportingEntity></ReportBody>";
    ...
    }// end translateReport009()
    
    // methods to restructure 009 into 100
    
    /**
     * creates a reportID in UUID format based on increasing static value
     */
    String nextReportID(){
        
        // increament the static value
        ersatzReportIndex++;
        
        // make a pattern to return
        return "00000000-0000-0000-0000-" + String.format("%012d",ersatzReportIndex);
    }
