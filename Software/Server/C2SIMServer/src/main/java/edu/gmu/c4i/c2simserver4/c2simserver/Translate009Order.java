 ...
 *
 * this class does 0.0.9 Order to 1.0.0
 *
 ...
 * @author jmarkpullen
 */
public class Translate009Order {
    ...
    String translate(String xml){
        ...
        outputXml +=
            "<IssuedTime><IsoDateTime>" + issuedTime + 
            "</IsoDateTime></IssuedTime><OrderID>" + orderId + "</OrderID>";
        
        // process Tasks one at a time
        while(processTask());

        // complete order
        outputXml += removeChunk("</OrderBody","</MessageBody>");
    ...
    boolean processTask(){
        ...
        String location = copySubChunk(task,"<Location>","</Location>");
        ...
        taskOut += "<Location><GeodeticCoordinate>" + altitude + 
            "<Latitude>" + latitude + "</Latitude><Longitude>" + longitude + 
            "</Longitude></GeodeticCoordinate></Location>";
        
        // do the same for the RouteLocations - in 100 they become
        // additional Locations - delete the pieces from Task as we go
        String routeLocation = removeTaskChunk("<RouteLocation","</RouteLocation>");
        while(!routeLocation.equals("")){
            latitude = extractValue(routeLocation,"<Latitude>");
            longitude = extractValue(routeLocation,"<Longitude>");
            altitude = extractValue(routeLocation,"<AltitudeMSL>");
        if(!altitude.equals(""))
            altitude = "<AltitudeMSL>" + altitude + "</AltitudeMSL>";
        taskOut += "<Location><GeodeticCoordinate>" + altitude + 
            "<Latitude>" + latitude + "</Latitude><Longitude>" + longitude + 
            "</Longitude></GeodeticCoordinate></Location>";
            routeLocation = removeTaskChunk("<RouteLocation","</RouteLocation>");
        }
        ...
        String startTime = copySubChunk(task,"<Name>","</IsoDateTime>");
        ...
        taskOut +=
            "<Name>" + name + "</Name><UUID>" + uuid + "</UUID><AffectedEntity>" +
            affected + "</AffectedEntity><DesiredEffectCode>" + effectCode +
            "</DesiredEffectCode><PerformingEntity>" + performing + "</PerformingEntity>" +
            "<StartTime><DateTime>" + startTime + "</DateTime></StartTime>" +
            "<TaskActionCode>" + actionCode + "</TaskActionCode>" +
