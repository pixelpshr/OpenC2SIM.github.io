...
public class RouteLayer extends Layer implements MapMouseListener {
    ...
    void addToRouteLayer(
        ...
        if (bmlDocumentType == "C2SIM Report"){
            ...
            createOrderGraphicsC2SIM(omgraphics);
    	} 
    ...
    public ListEditableOMGraphics createOrderGraphicsC2SIM(
        ...
        for (int i = 0; i < stringArray.length; i++){	
            if(stringArray[i].equals(bml.c2simTaskTag) ||
               stringArray[i].equals(bml.c2simAsxTag) )
                ++numberOfTasks;
        }
        if(numberOfTasks == 0){
            bml.printError("C2SIMGUI graphics processing found no C2SIM Tasks to process");
            return null;
        }
        if(bml.debugMode)bml.printDebug(
        ...
        // number , startGDC, endGDC, lengthGDC, noOfPointGDC, task Name - control measures
        ...
        // Parsing an order string into tasks and drawing location information 
        if(bml.debugMode)bml.printDebug("============   Drawing C2SIM Order  ==================");
        for (int stringArrayIndex = 1; 
            ...
            // effective with C2SIMv11 there is not a separate Route
            // just a sequence of locations to be followed
            ...
            if ( ((stringArrayIndex == stringArray.length-1) ||
                bml.c2simTagCompare(stringArray[stringArrayIndex],bml.c2simTaskTag))){
                ...
                if(startGDC < endGDC && foundLat && foundLon) {
                    ...
                    noOfPointGDC = lengthGDC/4;
                    if(bml.debugMode)bml.printDebug(
                        "  It ends at : " + endGDC + " its Length is: " + 
        ...
        for (taskIndex=0; taskIndex < numberOfTasks ; taskIndex++){
            ...
            // start tempArray with current possition of the Unit
            if(milOrg != null) {
            ...
            for (int h = 0; h < descArray[taskIndex][4]; h++) {							
                ...
                // save the last two values in tempArray for cenetering
        ...
        for (taskIndex=0; taskIndex < numberOfTasks ; taskIndex++){
            ...
            if(!execAfterTask.equals("")){
                for(int i=0; i <= bml.routeLayerIndex; ++i){
                    ...
                    if(i < bml.routeLayerIndex)
                        prevRouteLayer = bml.routeLayers[i];
                    ...
                    for(int j=0; j < prevRouteLayer.numberOfTasks; ++j){
                        if(!prevRouteLayer.taskUuid[j].equals(execAfterTask))
                            continue;

                        // j holds index of the predecessor Task in prevRouteLayer 
                        tempArray[0] = prevRouteLayer.destLat[j];
                        tempArray[1] = prevRouteLayer.destLon[j];
                        break;
        ...
        } // end for taskIndex processing location information of an Order09
