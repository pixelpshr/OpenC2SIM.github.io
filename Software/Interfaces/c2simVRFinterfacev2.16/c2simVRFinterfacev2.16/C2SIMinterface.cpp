...
/*
VR-Forces C2SIM interface v2.16 
consistent with C2SIM_SMX_LOX_ASX_v11.xsd
	...
	Task* thisTask)
{
	...
	// get the geolocation of executing Unit from initialization, or
	// if this is a task-follows-task situation from the simulation
	...
	// start route at present object location if we have been able to find it
	...
	// add Locations from order Task to the route
	for (int pointNumber = 0; pointNumber < numberOfPoints; ++pointNumber) {
		...
		// display coordinates of this point
	...
	*/
	...
	// follow the route if there is one
		...
		std::cout << "Task:" << thisTask->taskName << " made Route:" << routeName <<
			" for Unit:" << taskUnit->name << " UUID:" << "taskUnit->vrfUuid" <<
