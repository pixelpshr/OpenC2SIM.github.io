...
VERSION 2.23 uses C++ClientLibv4.8.3.1 with libboost-*-vc140-*
...
The top-level directory c2simVRFv2.23 can be installed in C:\
or elsewhere. This is changed from v2.6, which had to be located 
the vrforces4.6.1/examples directory. 

v2.23 includes binaries for C++C2SIMClientLibv4.8.3.1, which is 
...
v2.23 is compatible with C2SIM v1.0.2. It accepts multiple
...
c2simVRFv2.23 accepts a command line parameter (number 9) that
...
There is a batch file runVRF-pRTI.bat that will start VRForces sim and GUI
when running with Pitch RTI. If you at fomModules to this file be sure 
to add in the start line for vrfGui.exe and the sim vrfSimHLA1516e.exe,
and also to the HLA version of VRForces command line near line 250 of 
main.cxx. The fomModules MUST match in all three files and they also
must match configurations of all other federates in the HL federation.

There are also example batch files for starting the c2simVRFinterface 
for DIS (runc2simVRF.bat) and HLA (runc2simVRFHLA1516e.bat). 

You might want to make desktop shortcuts for these batch files.

c2simVRFv2.23 uses these command line parameters;
...
Version 2.23 includes support for batching multiple <ReportContent>
