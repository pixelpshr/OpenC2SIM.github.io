...
C2SIMxmlHandler::C2SIMxmlHandler(bool useIbmlRef, bool displayDebugRef)
{
	...
	unitMap; unitNameMap; unitVrfUuidMap; allPhysicalRoutes;
	thisUnit = nullptr;
	actorCount = 0;
	taskMap; taskNameMap;
	orderIdMap;
...
// inserts a PhysicalRoute into allPhysicalRoutes returns true on error
bool  C2SIMxmlHandler::insertRoute(std::string routeUUID, PhysicalRoute* route) {
	if (allPhysicalRoutes[routeUUID] != nullptr) {
		displayError("ROUTE:" + routeUUID +
			" ALREADY ENTERED - DUPLICATES NOT ALLOWED");
		return true;
	}
	allPhysicalRoutes[routeUUID] = route;
	return false;
}

// extracts a route given its UUID
PhysicalRoute* C2SIMxmlHandler::retrieveRoute(std::string routeUUID) {
	return allPhysicalRoutes[routeUUID];
}
...
void C2SIMxmlHandler::parseC2SIMOrder() {
	...
	if (parsingC2simOrderEntity && latestTag == "Route") {
		if (parsingC2simOrderRoute) {
			displayError("NESTED ROUTE IN C2SIM ORDER");
			skipThisDocument = true;
			return;
		}
		parsingC2simOrderRoute = true;
		thisOrderRoute = new PhysicalRoute();
	}
...
void C2SIMxmlHandler::parseC2simInitialize() {
	...
	if (parsingPhysicalEntity) {
		...
		if (latestTag == "Route") {
			if (thisPhysicalRoute == nullptr) {
				...
				displayError("NESTED ROUTE IN INITIALIZTION");
				skipThisDocument = true;
			}
			return;
		}
	...
	if (parsingPhysicalEntity && latestTag == "Location") {
		if (parsingPhysicalLocation) {
			displayError("NESTED LOCATION IN ROUTE UUID " + thisRouteUUID);
			skipThisDocument = true;
		}
...
void C2SIMxmlHandler::endElement(const XMLCh* const name)
{
	...
	if (!foundFinalTag) {
		if (parsingC2simInitialize) {
			...
			// finish parsing Route
			if (endTag == "Route"){

				// Route in C2SIMInitialization
				parsingRoute = false;
				thisPhysicalRoute->routeUUID = thisRouteUUID;
				if (insertRoute(thisRouteUUID, thisPhysicalRoute)) return;
			...
			// Route in Order Entity
			else if (endTag == "Route" && parsingC2simOrderRoute) {
				parsingC2simOrderRoute = false;
				(insertRoute(thisOrderRouteUuid, thisOrderRoute));
				return;
			}
			...
			if (parsingC2simOrderEntityCoordinate && endTag == "GeodeticCoordinate") {
				thisOrderRoute->locations.push_back(thisOrderEntityLocation);
				thisOrderRoute->routeUUID = thisOrderRouteUuid;
				parsingC2simOrderEntityCoordinate = false;
				return;
			}
...
void C2SIMxmlHandler::characters(
	...
	if (parsingC2simOrder) {
		...
		if (parsingC2simOrderEntity) {
			if (latestTag == "UUID") {
				if (thisOrderRouteUuid != "") {
					displayError("ENTITY ROUTE HAS DUPLIACTE UUID:" + dataText + " IN C2SIM ORDER");
					skipThisDocument = true;
					return;
				}
				thisOrderRouteUuid = dataText;
				return;
			}

			// collect Latitude Longitude and Altitude into Location 
			if (parsingC2simOrderEntityCoordinate) {
				if (latestTag == "Latitude") {
					if (thisOrderEntityLocation->latitude != "") {
						displayError("MULTIPLE LATITUDE IN ORDER ROUTE " + thisOrderRouteUuid);
						skipThisDocument;
						return;
					}
					...
					if (thisOrderEntityLocation->longitude != "") {
						displayError("MULTIPLE LONGITUDE IN ORDER ROUTE " + thisOrderRouteUuid);
						skipThisDocument;
						return;
					}
					...
					if (thisOrderEntityLocation->latitude != "") {
						displayError("MULTIPLE ALTITUDEMSL IN ORDER ROUTE " + thisOrderRouteUuid);
						skipThisDocument;
						return;
					}
	...
	// C2SIMInitialization message - only one instance per run
			...
			if (parsingBlueForceSide) {
				...
				if (latestTag == "HostilityStatusCode") {
					if (blueForceSide->otherSide1HostilityCode == "")
						...
						displayError("THIS IMPLEMENTATION DOES NOT ALLOW > 2 FORCESIDERELATIONSS");
						skipThisDocument = true;
						return;
					}
				}
		...
		// Entity and all of Unit data or Route data
			...
			// parse Route UUID and Locations
			else if (parsingRoute) {

				// look for route UUID
				if (latestTag == "UUID") {
					if (thisRouteUUID == "") {
						thisRouteUUID = dataText;
					}
					else {
						displayError("DUPLICATE ROUTE UUID AFTER " + thisRouteUUID);
						skipThisDocument = true;
					}
					return;
				}
				...
				// look for Location components
					...
					if (latestTag == "Latitude") {
						if(thisPhysicalLocation->latitude == "") {
							...
							displayError("DUPLICATE LATITUDE IN ROUTE UUID " + thisRouteUUID);
							skipThisDocument = true;
						}
						return;
					}
					...
					// look for Longitude
						...
						if (thisPhysicalLocation->longitude == "") {
							...
							displayError("DUPLICATE LONGITUDE IN ROUTE UUID " + thisRouteUUID);
							skipThisDocument = true;
						}
						return;
					}
					...
					// look for ElevationAGL
						...
						if (thisPhysicalLocation->elevation == "") {
							...
							displayError("DUPLICATE ELEVATION IN ROUTE UUID " + thisRouteUUID);
							skipThisDocument = true;
						}
						return;
					}
		...
		// control message 
		if (parsingSystemCommandTypeCode) {
		...
		}//end if(parsingSystemCommandTypeCode)
